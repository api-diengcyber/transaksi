...
import { useMemberService } from '~/composables/useMemberService'; // Gunakan Member Service
...
const loadData = async () => {
    ...
    try {
        ...
        // Cocokkan ID Member
        arList.value = dataList.map(item => {
    ...
    <div class="min-h-screen flex flex-col bg-surface-50 p-4 md:p-6 font-sans">
        ...
        <div class="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                ...
                <p class="text-sm text-surface-500 mt-1">Pantau sisa tagihan pelanggan, DP, dan histori pembayaran (cicilan).</p>
            </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            <div class="bg-white p-4 rounded-2xl shadow-sm border border-surface-200 flex items-center gap-4 border-l-4 border-l-orange-500">
                <div class="w-12 h-12 rounded-full bg-orange-50 flex items-center justify-center text-orange-500 shrink-0"><i class="pi pi-money-bill text-xl"></i></div>
                <div><div class="text-xs font-semibold text-surface-500 uppercase tracking-wider mb-1">Total Nilai Penjualan Kredit</div><div class="text-xl font-black text-surface-900">{{ formatCurrency(summary.totalPiutang) }}</div></div>
            </div>
            <div class="bg-white p-4 rounded-2xl shadow-sm border border-surface-200 flex items-center gap-4 border-l-4 border-l-emerald-500">
                ...
                <div><div class="text-xs font-semibold text-surface-500 uppercase tracking-wider mb-1">Total Pembayaran Masuk (DP+Cicil)</div><div class="text-xl font-black text-emerald-600">{{ formatCurrency(summary.totalTerbayar) }}</div></div>
            </div>
            <div class="bg-white p-4 rounded-2xl shadow-sm border border-surface-200 flex items-center gap-4 border-l-4 border-l-rose-500">
                <div class="w-12 h-12 rounded-full bg-rose-50 flex items-center justify-center text-rose-500 shrink-0"><i class="pi pi-exclamation-circle text-xl"></i></div>
                <div><div class="text-xs font-semibold text-surface-500 uppercase tracking-wider mb-1">Sisa Piutang (Belum Lunas)</div><div class="text-xl font-black text-rose-600">{{ formatCurrency(summary.totalSisaPiutang) }}</div></div>
            </div>
        </div>
            ...
            <div class="p-4 border-b border-surface-100 flex flex-col xl:flex-row justify-between items-center gap-4 bg-surface-0/50">
                <div class="flex flex-col sm:flex-row items-center gap-3 w-full xl:w-auto">
                    <div class="relative w-full sm:w-72">
                        ...
                        <input v-model="searchQuery" @input="onSearch" type="text" placeholder="Cari Faktur atau Pelanggan..." class="w-full pl-10 pr-4 py-2 border border-surface-200 rounded-xl text-sm focus:ring-orange-500 focus:border-orange-500 transition-colors" />
                    </div>
            ...
            >
                <template #empty>
                    <div class="flex flex-col items-center justify-center py-16 px-4 text-surface-500">
                        ...
                        <h3 class="text-lg font-bold text-surface-700">Belum Ada Transaksi Piutang</h3>
                        <p class="text-sm mt-1 max-w-sm text-center">Data piutang tidak ditemukan berdasarkan pencarian/tanggal.</p>
                    </div>
                </template>
                ...
                <Column field="code" header="No Faktur / Transaksi" sortable style="min-width: 14rem">
                    <template #body="{ data }">
                        <div class="flex flex-col gap-1 py-1">
                            <div class="font-bold font-mono text-orange-700 bg-orange-50 px-2 py-0.5 rounded-md inline-block w-max border border-orange-100 shadow-sm" v-tooltip.top="'Nomor Faktur / Invoice'">
                ...
                <Column field="remaining" header="Sisa Tagihan (Piutang)" style="min-width: 10rem">
                    <template #body="{ data }">
                        <div class="font-black text-sm text-rose-600">{{ formatCurrency(data.remaining) }}</div>
                        ...
                        <div class="bg-white p-4 rounded-xl border border-surface-200 shadow-sm">
                            <div class="flex justify-between items-center mb-3 border-b border-surface-100 pb-2">
                                <h5 class="text-sm font-bold text-emerald-800 flex items-center gap-2 m-0">
                                    <i class="pi pi-wallet"></i> Histori Pembayaran & Cicilan Piutang
                                </h5>
                            ...
                            <div class="space-y-2">
                                ...
                                <div v-else class="p-2 text-center">
                                    <span class="text-[10px] text-surface-400 italic">Belum ada cicilan/pembayaran masuk setelah DP.</span>
                                </div>
                            </div>
                            
                            <div class="mt-4 pt-3 border-t border-dashed border-surface-200 flex justify-between items-center bg-rose-50 p-2 rounded">
                                <span class="text-xs font-bold text-surface-700">TOTAL KEKURANGAN PELANGGAN</span>
                                <span class="font-black text-rose-600 text-lg">{{ formatCurrency(slotProps.data.remaining) }}</span>
                            </div>
                        </div>
                    </div>
                </template>
...
.p-datatable .p-datatable-tbody > tr.p-highlight {
    background-color: #fff7ed !important; /* Tema orange muda saat row di-expand */
}
