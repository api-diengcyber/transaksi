...
@Injectable()
export class JournalReturnBuyService {
  constructor(
  ...
  async createReturnBuy(payload: any, userId: string, storeUuid: string, externalManager?: EntityManager) {
    if (!storeUuid) throw new BadRequestException('Store ID is required.');

    const work = async (manager: EntityManager) => {
        // Generate kode dengan prefix RET_BUY
        const code = await this.journalService.generateCode('RET_BUY', storeUuid);
        ...
        // 3. Ekstrak Item Retur dan Lakukan STOCK OUT (Barang dikembalikan ke Supplier)
        if (dataDetails.items && Array.isArray(dataDetails.items)) {
            const stockItemsToMin: any[] = [];
            ...
            for (let index = 0; index < dataDetails.items.length; index++) {
                ...
                // Tambahkan ke keranjang pengurangan stok (Stock Out)
                stockItemsToMin.push({
                    productUuid: item.product_uuid,
                    variantUuid: item.variant_uuid,
                    unitUuid: item.unit_uuid,
                    warehouseUuid: finalWarehouseUuid,
                    qty: Number(item.qty_return || item.qty || 1) // Kuantitas yang diretur
                });
            ...
            // Eksekusi Stock Out
            if (stockItemsToMin.length > 0) {
                await this.journalStokService.stockOut(
                    stockItemsToMin, 
                    userId, 
                    manager,
                    storeUuid, 
                    `Retur Pembelian: ${code} (Ref: ${dataDetails.reference_journal_code || '-'})`
                );
            }
        }
        ...
        return { message: 'Return Buy journal created successfully', journal, details: detailEntities };
    };
  ...
  async getReport(storeUuid: string, queryParams?: any) {
    ...
    // 1. Query builder untuk mengambil jurnal RET_BUY
    const qb = this.dataSource.manager.createQueryBuilder(JournalEntity, 'journal')
      ...
      .andWhere('journal.code LIKE :ret', { ret: `%RET_BUY%` });
    ...
    const allReturns = formattedJournals.map(ret => {
      // Ekstrak Supplier / Vendor
      let supplierName = ret.detailsMap['supplier'] || ret.detailsMap['supplier_name'] || ret.detailsMap['vendor'];
      let supplierUuid = ret.detailsMap['supplier_uuid'] || ret.detailsMap['vendor_uuid'];
      ...
      while (ret.detailsMap[`item_name#${idx}`] || ret.detailsMap[`product_name#${idx}`] || ret.detailsMap[`name#${idx}`]) {
          items.push({
              ...
              price: Number(ret.detailsMap[`buy_price#${idx}`] || ret.detailsMap[`price#${idx}`] || 0),
    ...
    if (search) {
        ...
        filteredReturns = allReturns.filter(r => 
            ...
            r.supplier.toLowerCase().includes(lowerSearch)
        );
    }
